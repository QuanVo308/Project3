...
def update_pops():
    ...
    for i in pops:
        ...
        if ( i.name != get_pop_name(i, i.name[3], i.name[4:])
        or i.ring != get_pop_ring(i) 
        or not validate_pop(i) 
        or i.vlan_PPPoE != get_pop_vlan(i)
        or i.range_ip != get_pop_rangeIP(i)):
            i.name = get_pop_name(i, i.name[3], i.name[4:])
            i.ring = get_pop_ring(i)
            i.vlan_PPPoE = get_pop_vlan(i)
            i.range_ip = get_pop_rangeIP(i)
            print("update", validate_pop(i))
            i.save()
        print()
    print('ok')